...
Fixpoint compare_rec (T : ml_type) (h : nat) :=
  if h is h.+1 then
    ...
    | ml_array T1 =>
      fun x y =>
        compare_ref (ml_list T1) (compare_list (compare_rec T1 h)) x y
    | ml_list T1 => fun x y => compare_list (compare_rec T1 h) x y
    ...
    | ml_tree T1 T2 =>
      fun x y =>
        match x, y with
        | Leaf x1, Leaf y1 => compare_rec T1 h x1 y1
        | Node x1 x2 x3, Node y1 y2 y3 =>
          lexi_compare (compare_rec (ml_tree T1 T2) h x1 y1)
            (Delay
               (lexi_compare (compare_rec T2 h x2 y2)
...
Definition it_1 := (do it <- K it; fib 100000 10%int63) empty_env.
...
Definition it_2 := (do it_1 <- K it_1; ack 100000 3%int63 7%int63) empty_env.
Eval vm_compute in it_2.

Definition it_3 :=
  (do it_2 <- K it_2; ml_lt ml_string 100000 "hellas"%string "hello"%string)
    empty_env.
Eval vm_compute in it_3.

Definition cmp :=
  (do it_3 <- K it_3; ml_lt ml_char 100000 "a"%char "A"%char) empty_env.
...
Definition arr :=
  (do it_4 <- K it_4; newarray ml_int 3%int63 5%int63) empty_env.
...
Definition it_6 :=
  (do it_5 <- K it_5; ml_ge ml_color 100000 Green Blue) empty_env.
...
Definition it_7 :=
  (do it_6 <- K it_6;
   ml_lt (ml_tree ml_string ml_int) 100000
     (mknode ml_string
...
Definition it_8 := (do it_7 <- K it_7; fib2 100000 1000%int63) empty_env.
...
Definition it_9 :=
  (do it_8 <- K it_8; iota 100000 1%int63 10%int63) empty_env.
...
Definition it_10 := (do fib_1 <- K fib_1; fib_1 10%int63) empty_env.
...
Eval vm_compute in (do z <- K z; do it_11 <- K it_11; Ret z) empty_env.
