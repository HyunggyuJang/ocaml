      ...
      | Psig_class sdecls | Psig_class_type sdecls ->
          ...
          map_rec (fun rs decl ->
            let open Typeclass in [
              ...
              Sig_type(decl.clsty_ty_id, decl.clsty_abbr, rs, Exported);
            ]
          ) decls [rem]
          |> List.flatten
      | _ ->
    ...
    | item :: srem ->
        ...
        | Psig_class cl ->
            ...
            List.iter (fun cls ->
              ...
              Signature_names.check_type names loc cls.cls_ty_id;
            ...
            let sg =
              map_rec (fun rs cls ->
                ...
                [Sig_class(cls.cls_id, cls.cls_decl, rs, Exported);
                 ...
                 Sig_type(cls.cls_ty_id, cls.cls_abbr, rs, Exported)]
              ) classes [rem]
              |> List.flatten
            in
            let typedtree =
        ...
        | Psig_class_type cl ->
            ...
            List.iter (fun decl ->
              ...
              Signature_names.check_type names loc decl.clsty_ty_id;
              Env.register_uid
            ...
            let sg =
              map_rec (fun rs decl ->
                ...
                [Sig_class_type(decl.clsty_ty_id, decl.clsty_ty_decl, rs,
                                Exported);
                 Sig_type(decl.clsty_obj_id, decl.clsty_obj_abbr, rs, Exported);
                 Sig_type(decl.clsty_ty_id, decl.clsty_abbr, rs,
                          Exported)
                ]
              ) classes [rem]
              |> List.flatten
  ...
  let type_str_item env shape_map {pstr_loc = loc; pstr_desc = desc} =
    ...
    | Pstr_class cl ->
        ...
        let shape_map = List.fold_left (fun acc cls ->
            ...
            |> map Shape.Map.add_type cls.cls_ty_id
          ) shape_map classes
        in
        Tstr_class
          (List.map (fun cls ->
    ...
    | Pstr_class_type cl ->
        ...
        let shape_map = List.fold_left (fun acc decl ->
            ...
            |> map Shape.Map.add_type decl.clsty_ty_id
          ) shape_map classes
        in
        Tstr_class_type
          (List.map (fun cl ->
